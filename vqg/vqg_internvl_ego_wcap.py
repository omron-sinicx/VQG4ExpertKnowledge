...
log_path = Path(f"log_formal/log_internvl_ego_wcap.txt")
training_samples = []
...
for video in tqdm(comm_data):
    ...
    for entry in video["annotations"]:
        try:
            video_path = entry["video"].split(".")[0] + "_ego.mp4"
        except KeyError:
        ...
        with torch.no_grad():
            ...
            question1 = f"""You are an AI assistant tasked with generating insightful questions. You will be observing a task video. You will also be provided with a narration of the video with timestamp, which may include descriptions of actions.
...
##Should not
    ...
    5. DO NOT mention the existance of the narration.
...
out_path = Path("log_formal/internvl_ego_wcap.json")
