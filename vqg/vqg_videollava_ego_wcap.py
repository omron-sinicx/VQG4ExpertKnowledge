...
log_path = Path(f"log_formal/log_llavavideo_ego_wcap.txt")
training_samples = []
...
for video in tqdm(comm_data):
    ...
    for entry in video["annotations"]:
        ...
        with torch.no_grad():
            ...
            prompt = f"""USER: <video>You are an AI assistant tasked with generating insightful questions. You will be observing a task video. You will also be provided with a narration of the video with timestamp, which may include descriptions of actions.
...
out_path = Path("log_formal/qa_videollava_ego_wcap.json")
