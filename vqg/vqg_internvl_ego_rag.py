...
log_path = Path(f"log_formal/log_internvl_ego_rag.txt")
training_samples = []
...
for video in tqdm(comm_data):
    ...
    for entry in video["annotations"]:
        ...
        with torch.no_grad():
            ...
            question1 = f"""You are an AI assistant tasked with generating insightful questions. You will be observing a task video in an egocentric view.
...
out_path = Path("log_formal/qa_internvl_ego_rag.json")
