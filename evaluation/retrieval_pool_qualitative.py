...
# 根据 best_rank 对结果进行排序
# best_positive：排名数值越小表示越好
sorted_best = sorted(detailed_results, key=lambda x: x["best_rank"])
# worst_positive：排名数值越大表示越差
sorted_worst = sorted(detailed_results, key=lambda x: x["best_rank"], reverse=True)
# best_entropy：熵值越小表示越好
sorted_entropy = sorted(detailed_results, key=lambda x: x["entropy"])
# worst_entropy：熵值越大表示越差
sorted_worst_entropy = sorted(detailed_results, key=lambda x: x["entropy"], reverse=True)
# best_uncertainty：不确定性越小表示越好
sorted_best_uncertainty = sorted(detailed_results, key=lambda x: x["uncertainty"])
# worst_uncertainty：不确定性越大表示越差
sorted_worst_uncertainty = sorted(detailed_results, key=lambda x: x["uncertainty"], reverse=True)

output_file = "evaluation_results_"+args.filename+".txt"
...
with open(output_file, "w", encoding="utf-8") as f:
    ...
    f.write(f"Mean R: {mean_rank:.2f}\n")
    f.write(f"Median R: {median_rank}\n\n")
    f.write(f"Mean Entropy: {statistics.mean(entropy_values):.4f}\n\n")
    f.write(f"Mean Norm Entropy: {statistics.mean(norm_entropy_values):.4f}\n\n")
    f.write(f"Mean Uncertainty: {statistics.mean(uncertainty):.4f}\n\n")
    
    f.write("Good Samples：\n")
    for entry in sorted_best[:50]:
        f.write(f"\nClip: {entry['clip']}\n")
        f.write(f"Q: {entry['question']}\n")
        f.write(f"Best Rank: {entry['best_rank']}\n")
        f.write(f"GT: {', '.join(entry['correct_answers'])}\n")
        f.write("Top-5 Result：\n")
        for i, comment in enumerate(entry["top10"], start=1):
            f.write(f"  {i}: {comment}\n")
    
    f.write("\nWorse samples：\n")
    for entry in sorted_worst[:10]:
        f.write(f"\nClip: {entry['clip']}\n")
        f.write(f"Q: {entry['question']}\n")
        f.write(f"Best Rank: {entry['best_rank']}\n")
        f.write(f"GT: {', '.join(entry['correct_answers'])}\n")
    ...
    f.write("\nBest Entropy Samples：\n")
    for entry in sorted_entropy[:10]:
        f.write(f"\nClip: {entry['clip']}\n")
        f.write(f"Q: {entry['question']}\n")
        f.write(f"Best Rank: {entry['best_rank']}\n")
        f.write(f"GT: {', '.join(entry['correct_answers'])}\n")
        f.write(f"Entropy: {entry['entropy']:.4f}\n")
        f.write(f"Norm Entropy: {entry['norm_entropy']:.4f}\n")
        f.write("Top-5 Result：\n")
        for i, comment in enumerate(entry["top10"], start=1):
            f.write(f"  {i}: {comment}\n")
    
    f.write("\nWorst Entropy Samples：\n")
    for entry in sorted_worst_entropy[:50]:
        f.write(f"\nClip: {entry['clip']}\n")
        f.write(f"Q: {entry['question']}\n")
        f.write(f"Best Rank: {entry['best_rank']}\n")
        f.write(f"GT: {', '.join(entry['correct_answers'])}\n")
    ...
    f.write("\nBest Uncertainty Samples：\n")
    for entry in sorted_best_uncertainty[:10]:
        f.write(f"\nClip: {entry['clip']}\n")
        f.write(f"Q: {entry['question']}\n")
        f.write(f"Best Rank: {entry['best_rank']}\n")
        f.write(f"GT: {', '.join(entry['correct_answers'])}\n")
        f.write(f"Uncertainty: {entry['uncertainty']:.4f}\n")
        f.write("Top-5 Result：\n")
        for i, comment in enumerate(entry["top10"], start=1):
            f.write(f"  {i}: {comment}\n")
    f.write("\nWorst Uncertainty Samples：\n")
    for entry in sorted_worst_uncertainty[:10]:
        f.write(f"\nClip: {entry['clip']}\n")
        f.write(f"Q: {entry['question']}\n")
        f.write(f"Best Rank: {entry['best_rank']}\n")
        f.write(f"GT: {', '.join(entry['correct_answers'])}\n")
