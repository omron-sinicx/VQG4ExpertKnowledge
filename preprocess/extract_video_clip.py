# This script extracts exo and ego video clips based on annotations and descriptions and the generated gold QA pairs.
...
desc_data = json.load(open("/workspace/data/data_egoexo/annotations/atomic_descriptions_val.json"))["annotations"]

desc_dict = defaultdict(list)

vid_root = "/workspace/data/data_egoexo"

comm = json.load(open("/workspace/data/data_egoexo/annotations/proficiency_demonstration_val.json"))["annotations"]
...
qa = json.load(open("log_formal/qa_val_samples_5_cleaned.json"))
merged_qa = []
...
for entry in merged_qa:
    ...
    for i, qa_item in enumerate(entry["annotations"]):
        try:
            ...
            ego_path = str(Path(vid_root) / indexed_comm[vid]["video_paths"]["exo1"])
...
# 保存
with open("log_formal/qa_val_samples_video_w_desc_test.json", "w") as f:
