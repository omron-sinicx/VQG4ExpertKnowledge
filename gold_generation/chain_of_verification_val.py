...
TEMPLATE_QUESTIONTYPE = {
    "good_executions": f"""The aim of the question is to uncover the notifiable actions or techniques.
        - If the Expert Commentary gives reasons for why the execution is good or methods of how to conduct this good execution, the question should lead to these reasons or methods based on the observed execution.
        - If the Expert Commentary describes how the execution is well conducted, the question should lead to these descriptions.""",
    "tips_for_improvement": f"""The aim of the question is to uncover the actions that are not optimal.
        - If the Expert Commentary gives a suggestion to improve the execution, the question should lead to these suggestions based on the observed execution, not asking questions as if you know the "Expert Commentary".
        - If the Expert Commentary describes how the execution is poorly conducted, the question should lead to these descriptions.""",
}
...
with open("qa_seed_val.json", encoding="utf-8") as f:
    seeds = json.load(f)
...
# output and log
training_samples = []
log_path = Path(f"log/log_formalizedQA_cove2_{args.entry}.txt")
...
##Should NOT
    1. DO NOT ask for general evaluations, summaries, or opinions, such as:
...
##Output format (there must be a prefix "[question]")
[question] ..."""
...
out_path = Path(f"log_formal/qa_val_samples.json")
