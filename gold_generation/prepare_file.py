...
# 输出结构
training_samples = []
log_path = Path(f"log{args.entry}.txt")
...
for video in tqdm(seeds):
    ...
    for entry in video["annotations"]:
        ...
        A_hat = re.search(r"\[answer\]\s*(.+)", A_hat).group(1).strip()
...
# 保存输出
out_path = Path(f"qa_training_samples_from_seed_entry_{args.entry}.json")
